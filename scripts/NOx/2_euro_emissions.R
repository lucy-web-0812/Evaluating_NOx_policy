...
petrol_energy_density <- 10.57 # This is kWh/l - kilowatt hr per litre of fuel 
diesel_energy_density <- 10.57 
...
petrol_fuel_eff_lcv <- 4.6 # changing around with these values 
petrol_fuel_eff_motorcycles <- 4
petrol_fuel_eff_lgv <- 31.4


diesel_fuel_eff_lcv <- 6.4
diesel_fuel_eff_lgv <- 31.4
diesel_fuel_eff_hdde <- 30.7
...
converted_units_data <- left_join(euro_emissions_long, avg_new_car_fuel_consumption, by = "reg_date") |> 
  ...
  mutate(petrol_fuel_eff = ifelse(`Type of Vehicle` == "Motor Cycles", petrol_fuel_eff_motorcycles, petrol_fuel_eff)) |> 
...
ggplot(current_standards) +
  ...
  scale_x_continuous(name = "NOx Emission Limit Value (mg/kWh)", expand = c(0,0), limits = c(0,1000)) + 
...
vehicle_classes_to_match_name_labels <- c("Passenger Vehicle",
                                      "Passenger Vehicle",  
                                      "Motor Cycles", 
                                      "Light Commercial Vehicles 1,305 - 1760 kg reference mass", 
                                      "Heavy Duty Diesel Engines", 
                                      "Heavy Duty Diesel Engines")


power_df <- data.frame("vehicle_type" = vehicle_classes_to_match_name_labels, 
                       Fuel = c("Diesel", "Petrol", "Petrol", "Diesel", "Diesel", "Diesel"), 
                       label = c("Diesel Car", "Petrol Car", "Motorcycle", "Diesel Van", "Bus", "LGV"), 
                       power_kw = as.numeric(c("85", # Car - Diesel
                                               "81", # Car - Petrol
                                               "9",  # Motor Cycle 
                                               "101", # Diesel - Van
                                               "187", # Bus
                                               "350"))) # Heavy Duty Diesel Engines
...
current_standards_final <-   power_df |> 
  left_join(current_standards, by = c("vehicle_type", "Fuel")) |> 
...
collated_data <- current_standards_final |> 
  ungroup() |> 
  select(c(label, Fuel, nox_mgkwh, power_kw)) |> 
  mutate(Directive = "Euro Emissions") |> 
  rename(Product = label, `Power (kW)` = power_kw, `NOx (mg/kWh)` = nox_mgkwh ) |> 
  rbind(collated_data) |> 
  distinct()
