...
esg_data_with_average_capacities <- esg_data |> 
  ...
  mutate(power_gw = as.numeric(ifelse(`Site Name` %in% combined_esg_reports, average_of_installed_capacity_gw, `Total Generation Capacity (GW)`))) |> 
  mutate(`Power (kW)` = power_gw * 1000000) |> 
